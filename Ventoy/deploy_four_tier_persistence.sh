...
# 1. Dynamically Detect and Mount Partition 4 (NTFS or FAT32)
DATA_DEV=$(blkid -U "2C95D29B2DF0500E" 2>/dev/null || blkid -U "C9D1-3C83" 2>/dev/null || blkid -L "SHARED FAT" 2>/dev/null || echo "/dev/sdb4")
FS_TYPE=$(blkid -s TYPE -o value "$DATA_DEV" 2>/dev/null || echo "ntfs")

case "$FS_TYPE" in
    vfat|fat32)
        TARGET_MOUNT="/media/ubuntu/SHARED_FAT"
        MOUNT_CMD="mount -t vfat -o rw,umask=000,uid=1000,gid=1000,iocharset=utf8"
        ;;
    ntfs|fuseblk)
        TARGET_MOUNT="/media/ubuntu/2C95D29B2DF0500E"
        MOUNT_CMD="mount -t ntfs-3g -o rw,umask=000,uid=1000,gid=1000,force"
        ;;
    *)
        TARGET_MOUNT="/media/ubuntu/STORAGE"
        MOUNT_CMD="mount -o rw,umask=000,uid=1000,gid=1000"
        ;;
esac

if [ -b "$DATA_DEV" ]; then
    CURRENT_MOUNT=$(lsblk -no MOUNTPOINT "$DATA_DEV" 2>/dev/null | head -n1)
    if [ -z "$CURRENT_MOUNT" ]; then
        mkdir -p "$TARGET_MOUNT"
        $MOUNT_CMD "$DATA_DEV" "$TARGET_MOUNT" 2>/dev/null || mount "$DATA_DEV" "$TARGET_MOUNT" 2>/dev/null || true
        ACTUAL_MOUNT="$TARGET_MOUNT"
        echo "Mounted $DATA_DEV ($FS_TYPE) at $TARGET_MOUNT"
    else
        ACTUAL_MOUNT="$CURRENT_MOUNT"
        echo "$DATA_DEV is already mounted at $ACTUAL_MOUNT"
    fi
    ln -sfn "$ACTUAL_MOUNT" /home/ubuntu/shared_fat
    ln -sfn "$ACTUAL_MOUNT" /home/ubuntu/ntfs_usb
    ln -sfn "$ACTUAL_MOUNT" /home/ubuntu/Desktop/Data_Storage
fi
