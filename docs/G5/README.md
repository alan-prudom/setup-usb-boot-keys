...
This folder contains documentation, disaster recovery reports, and maintenance records specifically for the **HP ZBook 15u G5** (`AP-HP-G5` / `alan-USB-g5`).

---

## Document Index

* [DUAL_SENSOR_NVME_THERMAL_ARCHITECTURE_GUIDE.md](DUAL_SENSOR_NVME_THERMAL_ARCHITECTURE_GUIDE.md) — Architectural specification and forensic root-cause analysis of NVMe thermal crashes (`0x154` / `0xEF` / `0x7A`) and the 3-tier predictive thermal governor suite.
* [NVME_THERMAL_GOVERNOR_QUICK_START.md](NVME_THERMAL_GOVERNOR_QUICK_START.md) — Operator quick reference guide for the 4 monitoring interfaces (Session 0 background daemon, SSH console TUI, Safari web dashboard on port 8899, and Windows tray icon).
* [AUTOSTART_VERIFICATION_AND_TEST_PLAN.md](AUTOSTART_VERIFICATION_AND_TEST_PLAN.md) — Operational test plan, autostart service registration verification, and reboot recovery procedures for background daemon and user tray app.
* [DETAILED_RECOVERY_REPORT.md](DETAILED_RECOVERY_REPORT.md) — Detailed statement-by-statement report on recovering deleted WSL `ext4.vhdx` from a Clonezilla full-disk backup image over remote NAS (`192.168.1.34` on `/media/alan/home40`) using zero-disk-space sparse NBD architecture.
...
* [artifacts/](artifacts/) — Version-controlled archive of all conversational artifacts, checklists, session logs, and diagnostic reports saved under human-friendly filenames.
