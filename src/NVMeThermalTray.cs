...
namespace NVMeThermal
{
    public class TrayApp : Form
    {
        ...
        public TrayApp()
        {
            ...
            UpdateTrayIcon(45, 70, "Initializing...");
        ...
        private void RefreshState()
        {
            try
            {
                if (File.Exists(STATE_FILE))
                {
                    ...
                    int temp = ParseJsonInt(json, "temperatureC", 45);
                    ...
                    UpdateTrayIcon(temp, cpu, string.Format("NVMe: {0}°C | CPU: {1}% (Max {2}%)\nState: {3}", temp, cpu, ceiling, stateTag));

                    // Show Toast / Balloon Alert on Step-Down
                    if (lastNotifiedCpu != -1 && cpu < lastNotifiedCpu && cpu <= 60)
                    {
                        notifyIcon.ShowBalloonTip(4000, "🛡️ NVMe Thermal Safeguard", string.Format("Temperature reached {0}°C. Stepped DOWN CPU power: {1}% -> {2}%", temp, lastNotifiedCpu, cpu), ToolTipIcon.Warning);
                    }
                    lastNotifiedCpu = cpu;
                }
            }
        ...
        private void HandleRequest(HttpListenerContext ctx)
        {
            try
            {
                ...
                if (rawUrl == "/api/state")
                {
                    string json = File.Exists(STATE_FILE) ? File.ReadAllText(STATE_FILE) : "{}";
...
<html>
<head>
    <title>NVMe Thermal Governor & Monitor</title>
    ...
    <style>
        ...
        .card { background: #1e293b; border-radius: 16px; padding: 24px; max-width: 600px; width: 100%; box-shadow: 0 10px 25px rgba(0,0,0,0.5); border: 1px solid #334155; }
        ...
        .gauge-container { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px; }
        .gauge { background: #0f172a; border-radius: 12px; padding: 20px; text-align: center; border: 1px solid #334155; }
        .gauge-val { font-size: 48px; font-weight: bold; margin: 8px 0; }
        .gauge-label { color: #94a3b8; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; }
...
<body>
    <div class='card'>
        <div class='header'>
            <h1>🛡️ NVMe Thermal Governor</h1>
            <span class='status-pill' id='stateTag'>Connecting...</span>
        </div>
        <div class='gauge-container'>
            <div class='gauge'>
                <div class='gauge-label'>NVMe Temperature</div>
                <div class='gauge-val temp-cold' id='tempVal'>-- °C</div>
                <div style='font-size: 12px; color: #64748b;'>Samsung 512GB NVMe</div>
            </div>
            <div class='gauge'>
                <div class='gauge-label'>CPU Power Limit</div>
                <div class='gauge-val' style='color: #38bdf8;' id='cpuVal'>-- %</div>
                <div style='font-size: 12px; color: #64748b;' id='ceilingVal'>Max Ceiling: --%</div>
            </div>
        </div>
        <div class='meta'>
            ...
            <div><b>Probe Penalty Remaining:</b> <span id='penaltyVal'>--</span>s</div>
        </div>
    </div>
    <script>
        async function fetchState() {
            try {
                ...
                document.getElementById('tempVal').innerText = data.temperatureC + ' °C';
                document.getElementById('tempVal').className = 'gauge-val ' + (data.temperatureC >= 55 ? 'temp-hot' : data.temperatureC >= 51 ? 'temp-warm' : 'temp-cold');
                document.getElementById('cpuVal').innerText = data.cpuLimitPercent + ' %';
                document.getElementById('ceilingVal').innerText = 'Max Ceiling: ' + data.maxCeiling + '%';
